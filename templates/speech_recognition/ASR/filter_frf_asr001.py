...
tags = ['(())', '<ah>', '<breath>', '<click>', '<cough>', '<dtmf>', '<euh>',
        '<foreign>', '<hmm>', '<int>', '<laugh>', '<lipsmack>', '<ring>', '*']
...
for i, audiofile in enumerate(audiofiles):
    ...
    spkid = os.path.basename(os.path.dirname(audiofile))
    ...
    for tag in tags:
        ...
        if tag in text_orig:
            tag2file[tag].append((textname, text_orig))
...
for i, audiofile in enumerate(audiofiles_removed):
    ...
    spkid = os.path.basename(os.path.dirname(audiofile))
    output_dir = os.path.join(filtered_dir, spkid)
    ...
    print('{}/{}: move {} --> {}'.format(i+1, len(audiofiles_removed),
        os.path.splitext(audiofile)[0], os.path.splitext(audiofile_out)[0]))
...
#             os.path.splitext(audiofile_out)[0], os.path.splitext(audiofile)[0]))
